...
ALTER TABLE ai_phone.external_media_sources
  ADD COLUMN IF NOT EXISTS external_bridge_id text;

CREATE UNIQUE INDEX IF NOT EXISTS external_media_sources_bridge_idx
  ON ai_phone.external_media_sources(external_bridge_id)
  WHERE external_bridge_id IS NOT NULL;
...
BEGIN
  ...
  INSERT INTO ai_phone.external_media_sources(
    ...
    status, external_ingress_id, external_bridge_id, source_policy_version,
    idempotency_key, request_hash, source_url_hash, source_final_url_hash,
    ...
    event_payload->>'externalIngressId', event_payload->>'externalBridgeId',
    event_payload->>'sourcePolicyVersion', event_payload->>'idempotencyKey',
    event_payload->>'requestHash', event_payload->>'sourceUrlHash',
    event_payload->>'sourceFinalUrlHash', event_payload->>'sourceResolutionHash',
    ...
    external_ingress_id = EXCLUDED.external_ingress_id,
    external_bridge_id = EXCLUDED.external_bridge_id,
