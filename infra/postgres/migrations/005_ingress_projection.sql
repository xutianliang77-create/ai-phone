...
CREATE UNIQUE INDEX IF NOT EXISTS external_media_sources_idempotency_idx
  ON ai_phone.external_media_sources(session_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;
CREATE INDEX IF NOT EXISTS external_media_sources_active_idx
  ON ai_phone.external_media_sources(status, updated_at)
  WHERE status IN ('requested', 'ready', 'buffering', 'publishing', 'deleting');
...
BEGIN
  ...
  INSERT INTO ai_phone.external_media_sources(
    ...
    provider_operation_id, last_error_class,
    ...
    version = EXCLUDED.version, provider_operation_id = EXCLUDED.provider_operation_id,
