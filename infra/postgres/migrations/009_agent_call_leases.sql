...
ALTER TABLE ai_phone.agent_tasks
  ...
  ADD COLUMN IF NOT EXISTS worker_lease_token_hash text,
  ADD COLUMN IF NOT EXISTS worker_lease_expires_at timestamptz,
  ADD COLUMN IF NOT EXISTS worker_lease_attempt integer NOT NULL DEFAULT 0,
  ADD COLUMN IF NOT EXISTS provider_operation_id text
    REFERENCES ai_phone.provider_operations(id);

CREATE INDEX IF NOT EXISTS agent_tasks_dispatch_lease_idx
  ON ai_phone.agent_tasks(worker_lease_expires_at)
  WHERE status = 'dispatching';

CREATE INDEX IF NOT EXISTS agent_tasks_reconciliation_idx
  ON ai_phone.agent_tasks(updated_at)
