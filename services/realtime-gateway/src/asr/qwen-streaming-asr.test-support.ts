...
export class SyntheticQwenAsrSocket extends EventEmitter {
  ...
  send(raw:string,callback:(error?:Error)=>void){const e=JSON.parse(raw);this.sent.push(e);callback();this.onSend?.(e);
    if(e.type==="session.update"){this.language=e.session.input_audio_transcription.language??this.detectedLanguage??this.language;if(this.autoSetup)queueMicrotask(()=>this.receive({type:"session.updated",session:{id:"qwen-session",model:this.model,modalities:["text"],...e.session}}));}
