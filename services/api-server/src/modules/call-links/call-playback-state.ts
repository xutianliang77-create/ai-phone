...
export function applyCallPlaybackState(
  session: SessionRecord | null,
  event: CallRoomDataEvent,
) {
  const status = eventStatus(event.type);
  if (!status) return unchanged<CallPlaybackDto | null>(null);
  const identity = playbackIdentity(event);
  if (!session || session.mode !== "call_link") {
    return unchanged<CallPlaybackDto | null>(null);
  }
  ...
  if (existing.status === status) return unchanged(existing);
...
export function applyCallBargeInState(
  session: SessionRecord | null,
  event: CallRoomDataEvent,
) {
  ...
  if (!session || !playback) return unchanged<CallPlaybackDto | null>(null);
  ...
  if (JSON.stringify(playback.bargeIn) === JSON.stringify(bargeIn)) {
    return unchanged(playback);
  }
  playback.bargeIn = bargeIn;
  return changed(playback);
}

export function interruptCallPlaybackState(
  session: SessionRecord | null,
  reason: CallPlaybackInterruptReason,
  endedAt: string,
) {
  if (!session?.playbacks?.length) return unchanged(session);
  let didChange = false;
  ...
  return didChange ? changed(session) : unchanged(session);
}

function playbackIdentity(event: CallRoomDataEvent) {
  if (!event.playbackId || !event.sourceLegId || !event.targetLegId ||
    !Number.isInteger(event.generation) || (event.generation ?? 0) < 1) {
...
function assertPlaybackBinding(
  ...
  if (existing.segmentId !== event.segmentId ||
...
function interruptReason(
  ...
  return reason === "barge_in" || reason === "session_end" ||
      reason === "superseded" || reason === "failure" || reason === "recovery"
    ? reason : "failure";
}
...
function changed<T>(result: T) {
  return { changed: true as const, result };
}

function unchanged<T>(result: T) {
  return { changed: false as const, result };
}
