...
- 后台 Work 只新增 `workId` 和 `deliveryAttemptId`；失效代次必须写成
  `turnGeneration`、`playbackGeneration` 或 `dispatchGeneration`，禁止新增裸
  `generation`。
...
- `playback.*` 继续表示服务端 sink/source 生命周期；只有匹配目标客户端、leg、Work、
  delivery attempt 和作用域 generation 的 `client.playback.ended` 才表示用户设备完成播放。
