...
export function evaluateRealtimeSpeakerTurnReadiness(input) {
  const errors = [];
  ...
  if (speakerIds.length < 2) errors.push(`Expected 2 speakers, got ${speakerIds.join(",") || "none"}`);
  if (turnIds.length < 2) errors.push(`Expected 2 turns, got ${turnIds.join(",") || "none"}`);

  const first = ordered[0];
  const switched = ordered.find((segment) =>
    first && segment.speaker.speakerId !== first.speaker.speakerId);
  if (!first || !switched) errors.push("No chronological speaker switch was persisted");
  ...
  const dominantLanguages = unique(ordered.map((segment) => segment.dominantLanguage)
    .filter(Boolean));
  if (!dominantLanguages.includes("zh") || !dominantLanguages.includes("en")) {
    errors.push(`Expected zh/en language profiles, got ${dominantLanguages.join(",") || "none"}`);
  }

  const diagnostics = detail?.diagnostics?.speakerTurns;
  if (!diagnostics || diagnostics.confirmedBoundaryCount < 1) {
