...
case "$ASR_SERVICE_PROVIDER" in
  qwen3_asr)
    write_env "$ASR_SERVICE_DIR/.env" \
      ...
      "ASR_QWEN3_CONVERSATION_MIN_AUDIO_MS=1000" \
      "ASR_QWEN3_ENDPOINT_SILENCE_MS=1100" \
      "ASR_QWEN3_CONVERSATION_ENDPOINT_SILENCE_MS=600" \
      "ASR_QWEN3_LISTENING_ENDPOINT_SILENCE_MS=1400" \
      "ASR_QWEN3_CALL_LINK_ENDPOINT_SILENCE_MS=600" \
...
if [ "$SPEAKER_SERVICE_ENABLED" = "true" ]; then
  ...
  write_env "$SPEAKER_SERVICE_DIR/.env" \
    ...
    "SPEAKER_ONSET=0.5" \
    "SPEAKER_OFFSET=0.5" \
    "SPEAKER_PAD_OFFSET_MS=0" \
    "SPEAKER_MIN_DURATION_ON_MS=100" \
    "SPEAKER_MIN_DURATION_OFF_MS=320" \
