...
function SessionTraceSection({ readable, token, tenantId }: {
  readable: boolean;
  token: string;
  tenantId: string;
}) {
  ...
  const [report, setReport] = useState<Loadable<EnterpriseSessionTraceReportResponse>>({ state: "idle" });
  const submit = async (event: React.FormEvent) => {
    ...
    try {
      setReport({ state: "ready", data: await api.getSessionTraceReport(token, tenantId, requested) });
    } catch (error) {
      setReport({ state: "failed", error });
    }
  };
  return <section className="dashboard-section" aria-labelledby="trace-report-title">
    <SectionHeader icon={enterpriseIcons.dashboard.trace} id="trace-report-title" title="会话链路报告"
      description="按明确 session ID 查询质量、Provider、usage/ledger 与审计链路。" />
    {!readable ? <StatusPanel state="forbidden" description="当前账号缺少 audit:read，未提供会话查询入口。" /> : null}
    {readable ? <form className="dashboard-trace-form" onSubmit={(event) => void submit(event)}>
      <label>Communication session ID<input value={sessionId} maxLength={200}
        onChange={(event) => setSessionId(event.target.value)} placeholder="输入服务端 session ID" /></label>
      <button className="button button--primary" disabled={!sessionId.trim() || report.state === "loading"}>
        <MaterialIcon name={enterpriseIcons.action.search} />查询
      </button>
    </form> : null}
    {report.state === "loading" ? <StatusPanel state="loading" description="正在读取会话链路报告。" /> : null}
    {report.state === "failed" ? <DashboardError error={report.error} operation="读取会话链路报告" /> : null}
...
function SessionReport({ report }: { report: EnterpriseSessionTraceReportResponse }) {
  ...
  return <div className="dashboard-report">
    <div className="dashboard-report-grid">
      <article><span>翻译覆盖率</span><strong>{coverage}</strong><small>{report.quality.segmentCount} segments</small></article>
      <article><span>P95 延迟</span><strong>{report.quality.p95LatencyMs === null ? "无样本" : `${report.quality.p95LatencyMs} ms`}</strong><small>{report.quality.latencySampleCount} samples</small></article>
      <article><span>Provider 操作</span><strong>{report.providerOperations.length}</strong><small>{report.quality.providerFailureCount} failed</small></article>
      <article><span>货币成本</span><strong>未配置</strong><small>{report.monetaryCost.reasonCode}</small></article>
    </div>
    {report.quality.status === "no_samples" ? <StatusPanel state="empty" title="暂无质量样本"
      description="服务端没有该会话的 segment；页面不绘制趋势或补零。" /> : null}
    <p className="dashboard-report__meta">{report.session.kind} · {report.session.status} · policy {report.session.policyVersion} · entitlement {report.session.entitlementVersion}</p>
    <div className="dashboard-traces" aria-label="关联 trace IDs">{report.traceIds.map((traceId) => <code key={traceId}>{traceId}</code>)}</div>
    {report.usage.length === 0 ? <StatusPanel state="empty" title="暂无会话用量"
      description="没有关联 usage event/ledger；货币金额保持未配置。" />
      : <div className="dashboard-table-wrap"><table className="dashboard-table"><thead><tr><th>用量类别</th><th>数量</th><th>来源</th><th>Trace</th></tr></thead>
        <tbody>{report.usage.map((item) => <tr key={item.eventId}><td>{usageCategoryPresentation[item.category].label}<small>{item.ledgerEntryId}</small></td>
          <td>{item.amount.toLocaleString("zh-CN")} {unitLabels[item.unit]}</td><td>{item.sourceType}<small>{item.sourceRef}</small></td>
          <td><code>{item.traceId}</code></td></tr>)}</tbody></table></div>}
  </div>;
}
