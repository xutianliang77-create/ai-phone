...
| 8 | `OPT-UI-001~005` 长会话、连接状态与发布呈现 | code-ready/in_progress；失败状态与字幕内容隔离已过 CPU 回归 | 小屏、横屏、200% 字体、深色、VoiceOver/TalkBack、长会话和真实 Profile/Release 截图 |
...
| 10 | `OPT-SCAN-001` 扫描目标语言与复杂版面 | 主链路通过；目标语言、4×4表格碰撞、宽度字号及极端堆叠回退已过 CPU 回归 | 真机换向状态正确；真实菜单、表格、斜拍及长短译文视觉验收通过 |
