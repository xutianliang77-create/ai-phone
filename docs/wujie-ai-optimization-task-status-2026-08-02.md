...
| 9 | `OPT-UI-006/007` 记录产品验收 + “我的”发布身份 | in_progress/code-ready；记录错误恢复、全文/纪要/术语/待办一级入口、窄屏大字体、发布身份及账号返回刷新已过 CPU 回归 | 真机确认四入口可达与术语/待办操作；真实分享面板；Profile 页面、bundle、诊断版本一致；真机登录/退出返回状态正确 |
