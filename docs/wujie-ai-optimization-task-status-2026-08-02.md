...
| 9 | `OPT-UI-006/007` 记录产品验收 + “我的”发布身份 | in_progress/code-ready；记录错误恢复、四个一级入口、空全文不遮蔽其他review、列表到长全文四层搜索连续性、段数导航/raw按需展开、端侧三层文本重载及四格式导出、窄屏大字体、发布身份及账号返回刷新已过 CPU 回归 | 真机确认搜索键盘/返回状态、四入口、长全文和术语/待办操作；真实分享面板及接收端编码；Profile 页面、bundle、诊断版本一致；真机登录/退出返回状态正确 |
