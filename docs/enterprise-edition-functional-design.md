...
`ENT-UI-004` 的业务状态现由单一 tenant 只读快照返回：有 `campaign:read` 才汇总活动/待派发任务，有
`support:read` 才汇总队列、等待/SLA、AI/人工会话、claim 和开放工单，有 `meeting:read` 才汇总预约/开通/
进行/结束中会议与在线参会者。缺少某业务 scope 时服务端不执行对应 SQL，并返回 forbidden section；前端不以
隐藏卡片代替授权。快照只展示计数和需要关注项，没有时间序列时不画趋势，仍不在客户端估算货币成本。
...
- 每个企业候选 commit 必须通过高置信 SAST、tracked/untracked 密钥扫描和生产依赖门禁；P0/P1
  发现不能使用例外放行。依赖临时例外只允许较低等级、精确版本和适用链路，并公开 owner、缓解和到期日。
- 正式候选还必须引用七天内、绑定相同 commit 的隔离 test/staging 渗透证据，至少覆盖未认证访问、tenant
  上下文伪造、跨租户、角色提权、webhook 签名/重放和 payload 上限。未执行或 evidence 验签失败时只能
  显示 `not_ready`，不能用单元测试或静态扫描替代。
- 浏览器跨域默认同源；生产若分离部署，只接受服务端显式配置的精确 HTTPS Origin，不允许 wildcard 或反射
  请求 Origin。安全扫描和渗透结果属于发布证据，不进入租户业务页面，也不向客户端返回源码、规则命中行或凭据。

### 10.1 企业候选发布材料

- 企业发布包独立于国内移动 App Store 材料，必须绑定唯一 release ID、版本、候选 Git commit 和
  image digest；“latest”、分支名或口头确认不能成为候选身份。
- 必须包含服务说明、发布说明、经批准 SLA、隐私与数据处理说明、管理员手册、运维/事件响应手册和
  发布清单。每份文件保存相对路径、SHA-256、审批人和审批时间，内容变化后原批准立即失效。
- A0–A3、H1–H3 各有且只有一份通过证据，证据的 commit、image、reviewer、时间和文件 hash 必须与
  当前候选一致；主产品、其他分支、旧候选、本地 mock 或静态检查不能继承为企业放行证据。
- 产品、工程、安全、隐私、运维和法务六类审批必须分别存在。缺失、pending、时间无效或材料仍含
  草稿标记时，运行时和 CLI 都报告 `not_ready`。
- SLA 中的 availability、RPO/RTO 只能引用真实批准目标和同候选 H3 跨故障域实测；没有批准/实测时
  文档必须明确不承诺数值，不得用单机双库或同机恢复代替。
- 发布材料 checker 只验证候选绑定、文件、hash、证据和审批完整性，不生成验收证据，也不替代独立
  reviewer、法务判断、真实 Provider、PostgreSQL、浏览器或真机验收。
