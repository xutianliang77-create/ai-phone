...
本机机制回归必须在当前已完成31+55 migration、已创建两个空测试 tenant 的隔离 PostgreSQL 上执行：
...
一次有效演练至少保留：candidate commit/image、31+55 manifest、tenant/capability、前后 version、operation IDs、
