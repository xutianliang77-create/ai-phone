...
本机机制回归必须在当前已完成31+56 migration、已创建两个空测试 tenant 的隔离 PostgreSQL 上执行：
...
并发只计一次、open/probe/recovery/kill、事件 UPDATE SQLSTATE `55000` 和公共31段/enterprise 56段。
它不会调用真实 Provider，也不替代 kill SLO、告警或值班演练。

一次有效演练至少保留：candidate commit/image、31+56 manifest、tenant/capability、前后 version、operation IDs、
