...
- 记录详情“术语建议”确认。
- 系统高频错词表。
- 行业词库包：App 单选通用、商业、科技、医疗、旅游、餐饮或娱乐并持久化；API 校验后写入 realtime token 并返回实际词库版本；Gateway 按 session 选择统一生成 ASR hotwords/corrections、翻译 glossary 和 LLM 保护字段。旧客户端未提交选择时回退服务端默认包。
