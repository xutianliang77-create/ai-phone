...
| flush 不漏最后一句 | 代码和自动化门禁完成，待真实验收 | `packages/contracts/src/realtime/events.ts`、`services/realtime-gateway/src/connection/realtime-flush-tracker.ts`、`services/realtime-gateway/src/connection/websocket-server.final-flush.test.ts`、`apps/mobile/test/realtime_repository_final_flush_test.dart` | 需在 iPhone + 真实在线模型执行“说完立即结束”100 次，验证尾句保存成功率 >= 99%。 |
...
| 弹性字幕工作区 | 已完成 | `apps/mobile/lib/src/features/realtime/presentation/widgets/subtitle_timeline.dart`、`apps/mobile/lib/src/shared/widgets/auto_follow_scroll_view.dart`、`apps/mobile/test/subtitle_timeline_test.dart`、`apps/mobile/test/realtime_page_workspace_test.dart` | 真机复测 320dp、横屏、200% 字体、当前句、pending 和“回到底部”。 |
| 自动朗读译文 | 代码和自动化门禁完成，默认关闭待真实验收 | `apps/mobile/lib/src/features/realtime/presentation/controllers/speech_capture_gate.dart`、`apps/mobile/lib/src/features/realtime/presentation/pages/realtime_page_online_helpers.dart`、`apps/mobile/lib/src/features/realtime/presentation/widgets/realtime_settings_panel.dart`、`apps/mobile/ios/Runner/AudioSessionCoordinator.swift`、`apps/mobile/android/app/src/main/kotlin/com/example/translation_mobile/AudioSessionCoordinator.kt` | 扬声器使用播放期和尾音窗抑制，耳机保持连续采集，Listening 强制静音；需 iPhone/Android 连续 20 句确认不自激且不吞下一句后，再决定默认开放。 |
| 在线 TTS 顺序队列 | 代码和自动化门禁完成，待真实验收 | `services/realtime-gateway/src/tts/realtime-tts-output.ts`、`services/realtime-gateway/src/tts/http-tts-synthesizer.ts`、`apps/mobile/lib/src/features/realtime/presentation/controllers/realtime_controller_speech.dart` | 已覆盖 20 句顺序、单句失败降级、暂停/结束取消和 HTTP 中止；需 iPhone + VoxCPM2 验证字幕与听感顺序一致且结束后无残留声音。 |
| AudioSessionCoordinator | 代码和自动化门禁完成，待真实验收 | `apps/mobile/lib/src/platform/audio/audio_session_coordinator.dart`、`apps/mobile/lib/src/features/realtime/presentation/controllers/realtime_controller_audio_session.dart`、`apps/mobile/ios/Runner/AudioSessionCoordinator.swift`、`apps/mobile/android/app/src/main/kotlin/com/example/translation_mobile/AudioSessionCoordinator.kt` | 已统一采集/播放所有权、中断和路由事件；需覆盖电话/闹钟中断、扬声器、有线和蓝牙耳机真机恢复。 |
| 回声和播放策略 | 代码和自动化门禁完成，待真实验收 | `apps/mobile/lib/src/features/realtime/presentation/controllers/speech_capture_gate.dart`、`apps/mobile/test/speech_capture_gate_test.dart`、`apps/mobile/test/realtime_controller_echo_policy_test.dart`、`apps/mobile/test/realtime_settings_menu_test.dart` | 已按路由区分扬声器抑制与耳机连续识别，并强制 Listening 静音；需真实扬声器、听筒、有线及蓝牙耳机完成 AC-MOB-002。 |
