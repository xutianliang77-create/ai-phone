...
2026-07-12 隔离测试结论：`AC-VAD-002` 未通过，功能降级可继续 ASR，但缺 fallback 原因/次数、概率摘要、模型 fingerprint 和 session 诊断；`AC-VAD-003` 未通过，四模式均使用全局 `1100ms`，mode 尚未进入 ASR/VAD。证据见 `docs/vad-002-003-test-report-2026-07-12.md`。
...
| AC-LLM-002 | OPT-LLM-002 | raw、optimized、translated 可追溯且导出一致；端侧记录保存重载后三层文本不丢，Markdown/TXT/JSON/CSV 的扩展名、MIME 和内容与选择一致 |
| AC-LLM-003 | OPT-LLM-003 | 摘要、决定、待办、事实均有 evidence segment |
| AC-UI-006 | OPT-UI-006 | 历史列表展示标题、日期、时长、语言和摘要；两步内可查看纪要、全文、术语和待办；全文为空时已有 review 内容仍可达；列表与长全文均可搜索四层文本，列表快速输入以300ms合并为最后一次查询、键盘提交立即搜索、关闭搜索取消待发查询并恢复完整记录；打开字幕命中的记录后直接保留查询定位，纪要/待办/术语命中不得误清空全文；可查看匹配/总段数并按段展开 raw ASR；端侧重启后仍保留 raw/optimized/译文并可按所选格式导出 |
