...
ASR、翻译、TTS、说话人归属都必须通过统一接入层切换模型，业务代码不直接绑定某个模型：
...
Model services / external providers
        |
ASR model / translation model / TTS model / speaker model
```
...
| 说话人归属 | `/speaker/sessions` + `/speaker/frames` | 默认关闭，Sortformer v2.1 shadow | 其他 diarization Provider、RTC participant track |
...
- `profiles.*.speaker`：说话人 Provider、模型和协议；未通过 shadow 门禁时必须为关闭状态。
...
4. 运行对应 smoke：ASR、`check:translation-provider`、`check:tts-provider`、speaker DER/稳定性评测、LLM JSON 输出检查。
