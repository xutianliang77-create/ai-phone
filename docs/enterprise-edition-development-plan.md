...
- `ENT-DATA-005` 已形成 PostgreSQL-only 单租户跨 Cell 维护命令：签名 export/cutover/rollback evidence、
  ...
  测试、真实31+56双库、对象复制、路由发布和跨 Cell 故障演练未执行，保持 `in_progress`。
...
- `ENT-REL-003` 已形成 PostgreSQL 灾备静态候选：在任何 Provider step 前验签当前 enterprise cutover，
  绑定 commit/image/topology、目标数据库 identity 和31+56 manifest；schema-v2 签名结果要求自动切换 timeline/
...
- `ENT-REL-004` 已形成 `0052/0053` tenant-scoped 灰度/kill/circuit 真值、追加式 operation evidence、
  平台内部变更与专用 probe API，以及 Support Agent、Meeting Screen OCR、Marketing PSTN 三条服务端 guard。
  缺记录、过期、关闭、kill、open 和普通 half-open 全部拒绝；服务端 outcome 达阈值自动 open，只有第二探针
  密钥可推进 half-open。2026-07-21 已在本机 PostgreSQL 16.14 以普通角色完成双租户 RLS、两个连接池
...
- `ENT-REL-006` 已形成 `0054`、独立 control-plane role/session、实例/开通投影、SKIP LOCKED 多实例 Worker、
  generation/lease、异步202、配置失败闭合、live status 和运行手册；真实31+56 PostgreSQL、双实例/故障域、
  kill/network partition/区域自治未执行，保持 `in_progress`。
- `ENT-REL-007` 已形成 `0055`、候选绑定四能力policy、Cell/tenant并发/速率/有限队列、加权公平、runtime/operator
  functions、admin/reconcile Worker和Worker/PSTN/Screen Share三业务fence；真实RLS、公平、容量和soak未执行，
  保持 `in_progress`。
- `ENT-REL-008` 已形成 `0056`、签名 Provider event、可恢复 command、不可变 decision、账户/订阅转换、Cell Worker
  双租约、冻结 entitlement 安全排空、恢复新版本、usage period 重建、状态 API 和 health/release gate；测试、
  真实支付 Provider、31+56 PostgreSQL/RLS、Worker 崩溃、账期关账和财务抽样未执行，保持 `in_progress`。
...
- `ENT-MKT-003` 已形成 `0039`、对象存储证据实体/hash/大小/类型验证、Campaign/Lead/purpose 绑定、不可变
  Consent、幂等登记/撤回、执行时有效性查询、task insert/reschedule 数据库 guard 和同风格 Web 面板。当前只执行
  静态门禁，真实对象存储、PostgreSQL/RLS、并发撤回、浏览器和法务抽样仍待 `AC-ENT-0036`，状态保持
  `in_progress`；禁拨、国家策略、审批快照、Scheduler 和 PSTN 不在本任务内。
- `ENT-MKT-004` 已形成 `0040`、不可变 tenant/global Suppression、拒绝/撤回来源、tenant-scoped
...
- `ENT-MKT-006` 已形成 `0042/0043`、不可变 validation/decision、Campaign/Policy/Lead/Consent/Suppression
  snapshot/hash、validate/approve/reject API、状态迁移和 schedule/task SQL guard，以及按需加载的同风格 Web 审批面板。
  当前只形成静态候选，真实 PostgreSQL/RLS、审批与撤回/禁拨竞态、浏览器和法务抽样仍待 `AC-ENT-0039`；
  Scheduler、usage hold、Outbox、PSTN 继续由 `ENT-MKT-007..009` 实现。
- `ENT-MKT-007` 已形成 `0044`、approval snapshot 到确定性 call task 的同事务物化、当地时间 due query、
  `SKIP LOCKED`/CAS claim、当前 route epoch、租户/活动 entitlement 并发和固定60秒 usage hold 代码候选；租约到期、
  Consent 撤回与 Suppression 会释放未派发 hold 并收敛 task。Web 仅按需读取真实计数和阻断状态。本批未运行测试、
  PostgreSQL/RLS、50并发或浏览器；状态保持 `in_progress`。
- `ENT-MKT-008` 已形成 `0045`、scoped communication session/binding、事务 Outbox、稳定 Provider idempotency key、
  HTTPS PSTN Bridge Adapter、签名 webhook、Provider accept 后固定60秒 settle、旧 route/generation/provider call 栅栏和
  只读 Web dispatch 状态代码候选。prepare/finalize 分成两个 tenant transaction，外部调用位于事务外；响应未知保持
  reconciliation required 且不允许 lease reaper 盲目重拨。Provider/Bridge/webhook/keyring/idempotency 任一未配置即
  `not_ready`。本批未运行自动化、PostgreSQL/RLS、真实 Provider/PSTN、并发或浏览器，任务保持 `blocked`；其后续
  Marketing Agent 已由 `ENT-MKT-009` 单独实施。
- `ENT-MKT-009` 已形成版本化国家/locale profile、服务端 disclosure/qualifying/answering/closing 状态机、短期签名
...
- `ENT-MKT-010` 已形成 campaign/call tenant-scoped 只读监控投影、最终修订字幕、Agent 意图/风险、Provider 延迟/
  ...
  自动化、真实 PostgreSQL/RLS、真实通话、浏览器和负载未验收，保持 `in_progress`。
- `ENT-MKT-011` 已形成活动级 Support Queue/PSTN Channel/超时回拨策略、审批快照冻结、Marketing
  run/dispatch 到 Support Session 桥接、复用 exclusive claim 和坐席工作台媒体激活的代码候选。AI 数据库停播
  fence 与 Provider 真实停音/坐席加入回执分层，仅当 HTTPS、幂等、坐席加入和300ms保证均就绪时为 ready。
  超时只收敛为 `timed_out/callback_required`，不声称完成实体挂断/回拨。未运行自动化、`0047`/
  forced-RLS、真实 PostgreSQL/PSTN/坐席媒体、300ms或浏览器验收，保持 `in_progress`。
- `ENT-MKT-012` 已形成终态 task/dispatch/run、最终字幕/已交付 Agent turn、handoff/suppression/Provider 失败证据绑定，
...
7. `ENT-CS-011` 已形成工单/回拨、可靠 Outbox 和结果投影代码候选；下一阶段执行 AC-ENT-0032 的
   PostgreSQL、并发、崩溃恢复、浏览器和真实 Provider sandbox 验收。
...
9. `ENT-MKT-001/002/003/004/005/006` 已形成 Campaign 聚合、线索导入、不可变授权/禁拨/国家策略、活动审批快照与执行硬栅栏候选；
   下一阶段执行 `AC-ENT-0034..0038` 的 RBAC、E.164/身份去重、幂等、CAS/回滚、对象证据、撤回/拒绝、
   forced-RLS、时区/DST、同号码频控/审批竞态、浏览器和法务抽样；审批策略/数据快照已经固化。
...
- 租户级限流、配额、预算和异常熔断。
- 平台 tenant allowlist、能力级 kill switch、服务端 outcome 熔断和专用恢复探针；控制记录缺失或过期失败闭合。
- 数据保留、导出、删除、备份和恢复演练。
- SAST、依赖扫描、渗透测试和密钥轮换。
...
| 19 | `ENT-REL-007` | 在共享 Cell 配置四能力 policy，执行双租户权重公平、并发/速率/队列边界、三业务 lease、故障 reconcile、25/50/100 和120分钟混合容量验收 |
| 20 | `ENT-REL-008` | 配置签名账务 Adapter，执行 renew/past_due/suspend/recover/cancel、错签名/重放/逆序、双 Worker 崩溃重领、旧会话安全排空和账期 ledger count/hash 财务对账 |
