...
- `ENT-DATA-005` 已形成 PostgreSQL-only 单租户跨 Cell 维护命令：签名 export/cutover/rollback evidence、
  ...
  测试、真实31+55双库、对象复制、路由发布和跨 Cell 故障演练未执行，保持 `in_progress`。
...
- `ENT-REL-003` 已形成 PostgreSQL 灾备静态候选：在任何 Provider step 前验签当前 enterprise cutover，
  绑定 commit/image/topology、目标数据库 identity 和31+55 manifest；schema-v2 签名结果要求自动切换 timeline/
...
- `ENT-REL-006` 已形成 `0054`、独立 control-plane role/session、实例/开通投影、SKIP LOCKED 多实例 Worker、
  generation/lease、异步202、配置失败闭合、live status 和运行手册；真实31+55 PostgreSQL、双实例/故障域、
...
当前进展：`ENT-DATA-001` 已完成五十五段 schema 代码，等待 staging PostgreSQL migrate/restore/PITR
