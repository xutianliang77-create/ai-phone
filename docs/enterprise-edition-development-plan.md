...
- `ENT-DATA-001` 已实现三十九段 PostgreSQL up/down migration、复合 FK、强制 RLS、migration runner、schema verify 和归档 smoke；`0017..0039` 分别增加知识、术语/话术、企业 trace、审计导出、Meeting 聚合/入会/翻译、屏幕共享租约、会后材料、屏幕 OCR、日历同步、客服领域、Support Agent run/turn、Tool Registry、只读工具租约执行、可逆写确认/Outbox、高风险请求、坐席 claim、工单/回拨后续动作、客服质检证据、Campaign 聚合、线索导入与授权证据守卫。历史本地 PostgreSQL 16 验证不替代当前31+39 staging migrate/restore/PITR，任务保持 `in_progress`。
...
9. `ENT-MKT-001/002/003` 已形成 Campaign 聚合、线索导入与不可变授权证据代码候选；下一阶段执行
   `AC-ENT-0034/0035/0036` 的 RBAC、E.164/身份去重、幂等、CAS/回滚、对象证据、撤回、forced-RLS、密文和
   浏览器矩阵，随后从 `ENT-MKT-004` 接禁拨、国家策略与审批快照。
...
当前进展：`ENT-DATA-001` 已完成三十九段 schema 代码，等待 staging PostgreSQL migrate/restore/PITR
