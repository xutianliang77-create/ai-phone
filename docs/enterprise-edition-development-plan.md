...
- `ENT-DATA-001` 已实现十五段 PostgreSQL up/down migration、复合 FK、强制 RLS、migration runner、schema verify 和归档 smoke；`0014` 增加 usage budget/hold/alert，`0015` 增加 tenant billing account 与版本化 plan/subscription/entitlement，真实 PostgreSQL migrate/restore/PITR 尚无环境证据，任务保持 `in_progress`。
...
当前进展：`ENT-CORE-003` 已完成并等待验收；`ENT-DATA-001` 已完成十五段 schema 与本地自动化，等待真实 PostgreSQL migrate/restore/PITR 证据；`ENT-DATA-002/003/004/007/008` 已完成 Repository、可靠事件、演示导入、统一 Primary Runtime 和公共通讯 tenant scope 的代码与自动化；`ENT-CORE-013/014/015` 已完成企业统一通讯绑定、签名 Worker dispatch fence 及设备/声音/录制运行策略并进入验收；`ENT-CORE-007/010` 已完成 tenant budget、billing account、版本化 plan/subscription/entitlement 与 dispatch 服务端限额自动化并进入验收；`ENT-UI-001/002/003` 已进入验收。下一优先级为 `ENT-CORE-012` 的原始 usage event、账期聚合和 adjustment；`ENT-DATA-009` 随后负责最终切换和全量对账证据。
