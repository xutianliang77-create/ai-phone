...
- `ENT-DATA-001` 已实现四十三段 PostgreSQL up/down migration、复合 FK、强制 RLS、migration runner、schema verify 和归档 smoke；`0017..0041` 增加知识、会议、客服与 Campaign/Lead/Consent/Suppression/Country Policy，`0042/0043` 增加不可变活动 validation/decision snapshot 和调度/任务执行栅栏。历史本地 PostgreSQL 16 验证不替代当前31+43 staging migrate/restore/PITR，任务保持 `in_progress`。
...
当前进展：`ENT-DATA-001` 已完成四十三段 schema 代码，等待 staging PostgreSQL migrate/restore/PITR
