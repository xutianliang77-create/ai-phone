...
- `ENT-DATA-001` 已有五十六段 PostgreSQL up/down migration、tenant-first 索引、复合 FK、强制 RLS、checksum/锁和备份归档 smoke；`0017..0034` 延续既有知识、会议、客服 Agent/Tools、高风险接管和坐席 claim，`0035..0041` 增加客服后续动作/质检及 Campaign、Lead、Consent、Suppression、Country Policy，`0042/0043` 增加不可变活动校验/审批快照和执行栅栏，`0044` 增加 Scheduler claim/lease/hold 栅栏，`0045` 增加 scoped PSTN dispatch、Provider/Webhook 状态证据和任务转换硬栅栏，`0046` 增加 Marketing Agent profile/run/turn 栅栏，`0047` 增加审批冻结的人工接管策略、桥接证据和超时收敛栅栏，`0048` 增加 Outcome/内部 requested action 不可变证据栅栏，`0049` 增加 Salesforce CRM sync/receipt 栅栏，`0050` 增加 Cell Worker queue owner/generation/lease 协调栅栏，`0051` 增加对象生命周期账本与删除栅栏，`0052` 增加灰度/kill/circuit 控制和追加证据，`0053` 补齐 tenant root 自租户 forced-RLS policy，`0054` 增加控制面实例/开通投影多实例租约，`0055` 增加租户准入/共享Cell公平，`0056` 增加订阅/欠费生命周期。历史31+53证据已陈旧；当前31+56未执行真实 migration/down/forward，不能替代 staging restore/PITR/H3。
...
- `ENT-DATA-007` 已把上游稳定提交 `fe1c3c2` 的公共 Primary Runtime 纳入企业分支，并以 `API_STORAGE_DRIVER` 作为唯一进程级 driver。统一启动编排现验证公共31段和 enterprise 56段 manifest，核对数据库 name/OID 后才创建企业 runtime；API tenant pool 复用公共 Primary pool，directory、cell、control-plane、admission、migration 和 maintenance 使用分权连接配置，Worker 不获取 directory 凭证。代码和本地自动化完成，进入 `ready_for_acceptance`；当前56段真实 PostgreSQL 双 manifest、最小权限角色、并发和恢复证据仍属于 H3。
...
- `ENT-DATA-009` 已实现动态双 manifest、全业务表主键分页整行 count/SHA-256、关键 tenant/session/ledger/audit/consent/suppression/country-policy/approval/PSTN dispatch/Marketing Agent/Outcome/CRM/lifecycle/object 清单、WAL 水位、HMAC baseline/cutover/restore evidence、源库 SQLSTATE `25006` writer fence、旧 writer 会话清退和 production startup 身份绑定。提交 `c9b5be2` 的历史本地证据覆盖公共31段/企业16段与81张表；当前代码为31+56且新增全局控制面/admission表及tenant账务生命周期表，旧签名证据会被 startup gate 拒绝，必须在 staging 动态重算表数/hash。任务保持 `ready_for_acceptance`；跨故障域自动选主、异地主机不可变 WAL/PITR 和 RPO/RTO 仍待 `ENT-REL-003`/H3。
- `ENT-DATA-005` 已形成 `enterprise:postgres-cell export|cutover|reconcile|rollback` 代码候选。动态计划覆盖
  所有 enterprise tenant 表和公共 tenant communication scope；无 selector 表、无主键、schema 漂移、活动会话/
  租约、源未只读、旧 writer 未清退、目标非空、对象 receipt 缺失、逐表 count/hash 或 route epoch 不一致均
  失败闭合。普通 cutover 由 trigger 重建 pending projection；rollback 使用受审计 superuser 在单事务内反向
  全量替换最新数据。测试已定义但未运行，真实31+56 PostgreSQL、对象复制、控制面 route 和跨 Cell 演练均未执行，
  `AC-ENT-0048` 未通过，状态保持 `in_progress`。
- `ENT-DATA-006` 已形成 `0050` 与 Cell Worker 多实例协调代码候选：pending queue 使用数据库
...
- `ENT-REL-002` 已形成 `0051` forced-RLS 数据生命周期账本、audit export 完成自动登记、到期/租户删除
...
- `ENT-REL-003` 已将既有 resilience runner/checker 升级为 enterprise schema-v2 灾备证据门禁：独立 HMAC
  绑定当前 staging cutover、commit/image/topology、目标数据库 identity、公共31段/enterprise 56段 manifest；
...
- `ENT-REL-004` 已形成 `0052/0053` forced-RLS release control/event 与 tenant root policy、平台内部 change/decision/outcome API、
  第二 probe secret、CAS/advisory lock/operation ID 证据，以及 Support Agent、Meeting Screen OCR、Marketing PSTN
  副作用前 guard 和服务端 outcome 回写。租户角色只读，未配置/过期/disabled/kill/open/普通 half-open 失败闭合；
  threshold open 与 probe close/reopen 均有矩阵。2026-07-21 已在本机 PostgreSQL 16.14 以普通非 owner、非
  `BYPASSRLS` 角色完成双租户攻击、两个连接池同 operation 竞争、open/probe/recovery/kill 和追加证据不可变；
...
- `ENT-REL-006` 已形成 `0054` 控制面实例/开通投影、forced-RLS/trigger、独立最小权限 session、
  `SKIP LOCKED` 多实例 Worker、owner/generation/lease、PostgreSQL HTTP 异步202、配置缺失时开通/套餐变更
  fail-closed、同区域同候选副本/backlog status CLI 和运行手册。Cell 迁移排除全局控制面表并在活动 claim 时阻断；
  进行中区域会话不读取控制面进程状态。测试定义已增加但按要求未运行，当前31+56 migration、普通角色 RLS、
  双进程/故障域/kill -9/网络分区/滚动排空/真实 provision 和区域自治均未验收，任务保持 `in_progress`。
- `ENT-REL-007` 已形成 `0055` Cell/tenant admission policy/state/request、forced-RLS runtime functions、
  operator-only apply/weight/status/reconcile、候选绑定 policy manifest、独立 admission role/CLI/reconcile Worker，
  以及并发/速率/有限队列/weight virtual finish 算法。Worker Dispatch、Marketing PSTN、Screen Share 已在副作用前
  reserve/active-check，并随 accept/heartbeat/webhook/stop/revoke renew/release；Cell迁移排除全局 admission 表且活动请求
  直接阻断。测试定义已增加但按要求未运行，真实31+56、角色GRANT/RLS、双租户公平、Provider/媒体、25/50/100和
  120分钟soak均未验收，`AC-ENT-0056` 未通过，任务保持 `in_progress`。
...
- `ENT-UI-004` 已实现企业工作台服务端真值投影：租户/区域、Provider capability、subscription、预算告警、
  不可变 ledger 账期聚合、显式 session trace，以及 scope-aware Marketing/Support/Meeting 业务快照。业务快照不建
  第二套表，在同一 tenant 只读可重复读事务中分别汇总；无 scope 不执行该域 SQL，Web 以 generation/tenant key
  拒绝旧响应。预算只比较相同 category、unit 和 period，不跨单位求和；无价格表/质量样本时分别显示
  not_configured/no_samples，无时间序列不画趋势。Contracts/Web/API typecheck 和静态门禁通过；自动化、浏览器和
  PostgreSQL/RLS 测试按要求未执行，状态保持 `in_progress`。
...
- PostgreSQL runtime、cell Worker 和演示数据导入对账代码已接通，但尚未在真实 PostgreSQL 上执行 migrate/import/reconcile、并发租约、恢复或容量门禁，不能据此宣称企业试点或生产可用。受控审计导出的物理删除、实体复核和租户删除收敛已形成 `ENT-REL-002` 静态候选，但真实对象清单/Provider 验收仍未执行；会议/客服/营销业务聚合和外部 Provider 仍未通过真实环境门禁。
...
50 Scheduler 并发、故障隔离、浏览器与 staging 31+48 证据均未执行，`AC-ENT-0040` 未通过，状态保持 `in_progress`。
...
`AC-ENT-0044` 未通过，任务保持 `in_progress`。

`ENT-MKT-012` 已形成 `0048` 对既有 `marketing_outcomes` 的 forward-safe 升级和 forced-RLS
`marketing_next_actions`。Repository 只从终态 task/dispatch/run 建立 Outcome，验证最终字幕 revision、已交付 Agent turn、
handoff、suppression 和 Provider 失败依据，固化不含客户原文的 evidence/source hash；旧 `legacy_unverified` 行不进入读取，
但继续阻断同 task 重复生成。Outcome/action 均不可更新删除，以 advisory transaction lock、task 唯一键、幂等 request hash
和 deferred action constraint 阻断并发重复或不完整事务。后续动作当前只能是内部 `requested`，审计和页面明确
`externalAction=not_executed`，不伪造回拨、预约、资料发送或 CRM 成功。当前只完成代码、未运行测试定义和静态门禁候选；
自动化、`0048` migrate/down/forward、forced-RLS/双租户、真实 PostgreSQL/PSTN/Agent 通话和浏览器均未验收，
`AC-ENT-0045` 未通过，任务保持 `in_progress`。下一项为 `ENT-MKT-013` CRM Adapter。

`ENT-MKT-013` 已形成 `0049` forced-RLS `marketing_crm_syncs`、Outcome 单一同步约束、不可变
tenant/outcome/provider/external-key/payload/config fingerprint、AES-256-GCM Outbox 和 Worker 原子 finalize。
首个 Provider 为 Salesforce：只接受绑定租户和完整 OAuth Client Credentials、API version、sObject、External ID
与载荷字段配置，以稳定 External ID 执行 REST PATCH upsert，并在 GET 返回相同 ID/载荷后生成 receipt；401 重新取
token，429/5xx/传输未知保持 pending 指数退避，MKT-012 Outcome/通话终态和结算不回滚。Web 沿用 Material Icons
与既有 Outcome 面板，区分“等待回执/已对账/终态失败”，不把202或 Outbox 入队显示为成功。当前仅完成代码、
...
| ENT-REL-006 | SaaS 控制面高可用 | CORE-009/010/011 | `0054` instance/provision projection、独立 role、SKIP LOCKED Worker、generation/lease、status/runbook | 同区域至少两同候选实例；API/Worker 崩溃可恢复且不重复开通；控制面故障不破坏进行中会话 | in_progress |
| ENT-REL-007 | 租户限流和共享 Cell 公平 | CORE-010/014、OBS-001 | `0055` policy/state/request、并发/速率/有限队列、weight fair admission、三业务lease fence、admin/reconcile Worker | 大租户满载不头阻塞小租户；Cell不超限；队列/崩溃有界收敛且不重复副作用/结算 | in_progress |
| ENT-REL-008 | 订阅和欠费状态 | CORE-010/012 | `0056` signed event/command/decision、renew/past_due/suspend/resume/cancel、Cell Worker、status API/设置页、账期重建 | 不误停进行中安全链路，不漏账；错签名/重放/旧generation失败闭合 | in_progress |
...
`ENT-REL-006/007` 已有代码候选但 live HA、公平和容量演练未执行；配置 `configured`、静态 policy 或单机 status
仍不等于高可用/noisy-neighbor 已验收，REL-004 release control 也不冒充限流器。`ENT-REL-008` 已形成
`0056` forced-RLS provider event/command/decision、HMAC+时间窗内部入口、账户/订阅转换 trigger、Cell Worker
双租约、冻结 entitlement 排空、恢复新版本、账期 usage count/hash 重建、只读状态 API 和 health/release gate。
测试定义存在但按要求未执行；真实支付 Provider、31+56 PostgreSQL、双租户RLS、Worker崩溃、账期关账和财务
抽样未验收，因此保持 `in_progress`，未伪造 ready 或生产可用。
