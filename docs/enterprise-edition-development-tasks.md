...
- `ENT-DATA-001` 已有十五段 PostgreSQL up/down migration、tenant-first 索引、复合 FK、强制 RLS、checksum/锁和备份归档 smoke；`0014` 增加 tenant usage budget/hold/alert，`0015` 增加 tenant billing account、版本化 plan/subscription/entitlement 及 dispatch 归属。一次性本地 PostgreSQL 16 验证不替代真实 migrate/restore/PITR 证据，任务保持 `in_progress`。
