...
- `ENT-DATA-001` 已有十一段 PostgreSQL up/down migration、tenant-first 索引、复合 FK、强制 RLS、checksum/锁和备份归档 smoke；`0008` 增加 user directory，`0009` 增加 cell pending-work projection，`0010` 把 user/actor identity 从 UUID 修正为受约束的 opaque subject text，`0011` 增加企业通讯会话绑定。一次性本地 PostgreSQL 16 验证不替代真实 migrate/restore/PITR 证据，任务保持 `in_progress`。
