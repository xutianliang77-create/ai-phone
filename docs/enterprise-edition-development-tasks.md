...
- `ENT-DATA-001` 已有五十四段 PostgreSQL up/down migration、tenant-first 索引、复合 FK、强制 RLS、checksum/锁和备份归档 smoke；`0017..0034` 延续既有知识、会议、客服 Agent/Tools、高风险接管和坐席 claim，`0035..0041` 增加客服后续动作/质检及 Campaign、Lead、Consent、Suppression、Country Policy，`0042/0043` 增加不可变活动校验/审批快照和执行栅栏，`0044` 增加 Scheduler claim/lease/hold 栅栏，`0045` 增加 scoped PSTN dispatch、Provider/Webhook 状态证据和任务转换硬栅栏，`0046` 增加 Marketing Agent profile/run/turn 栅栏，`0047` 增加审批冻结的人工接管策略、桥接证据和超时收敛栅栏，`0048` 增加 Outcome/内部 requested action 不可变证据栅栏，`0049` 增加 Salesforce CRM sync/receipt 栅栏，`0050` 增加 Cell Worker queue owner/generation/lease 协调栅栏，`0051` 增加对象生命周期账本与删除栅栏，`0052` 增加灰度/kill/circuit 控制和追加证据，`0053` 补齐 tenant root 自租户 forced-RLS policy，`0054` 增加控制面实例/开通投影多实例租约。2026-07-21 本机31+53证据在新增 migration 后已陈旧；当前31+54未执行真实 migration/down/forward，不能替代 staging restore/PITR/H3，任务保持 `in_progress`。
...
- `ENT-DATA-007` 已把上游稳定提交 `fe1c3c2` 的公共 Primary Runtime 纳入企业分支，并以 `API_STORAGE_DRIVER` 作为唯一进程级 driver。统一启动编排现验证公共31段和 enterprise 54段 manifest，核对数据库 name/OID 后才创建企业 runtime；API tenant pool 复用公共 Primary pool，directory、cell、control-plane、migration 和 maintenance 使用分权连接配置，Worker 不获取 directory 凭证。代码和本地自动化完成，进入 `ready_for_acceptance`；当前54段真实 PostgreSQL 双 manifest、最小权限角色、并发和恢复证据仍属于 H3。
...
- `ENT-DATA-005` 已形成 `enterprise:postgres-cell export|cutover|reconcile|rollback` 代码候选。动态计划覆盖
  ...
  全量替换最新数据。测试已定义但未运行，真实31+54 PostgreSQL、对象复制、控制面 route 和跨 Cell 演练均未执行，
...
- `ENT-REL-003` 已将既有 resilience runner/checker 升级为 enterprise schema-v2 灾备证据门禁：独立 HMAC
  绑定当前 staging cutover、commit/image/topology、目标数据库 identity、公共31段/enterprise 54段 manifest；
...
- `ENT-REL-006` 已形成 `0054` 控制面实例/开通投影、forced-RLS/trigger、独立最小权限 session、
  ...
  进行中区域会话不读取控制面进程状态。测试定义已增加但按要求未运行，当前31+54 migration、普通角色 RLS、
...
`ENT-REL-006` 已有代码候选但 live status/故障演练未执行；配置 `configured` 或手工启动两个进程仍不等于
控制面高可用。`ENT-REL-007` 尚无租户 quota/共享 Cell 公平调度与容量保护实现，REL-004 release control
不能冒充限流器；`ENT-REL-008` 的 `/health/ready` 因必需支付 Provider 未配置返回503，尚无完整
renew/past_due/suspend/resume 生命周期。`ENT-REL-007/008` 继续保持 `todo`，未伪造 ready 或生产可用。
