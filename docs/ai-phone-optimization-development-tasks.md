...
2026-08-03 在“禁止使用GPU”的当前合同下，执行顺序调整为：`OPT-SCAN-001B OCR语言优先级真机门
-> OPT-UI-007真实账号返回 -> VoiceOver人工耳听 -> iOS最终发布门`；模型、speaker、尾句和endpoint
...
`001B code-ready（OCR语言优先级真机门待执行）`：
...
CPU真实菜单A/B和37项回归通过；需在隔离iPhone包复测后才能提交，不改写手写菜单能力边界。
