...
- `OPT-UI-004`：`in_progress（代码完成，真机验收待执行）`。新增浅色/深色设计 Token，区分主操作、错误和提醒色；卡片和 segmented control 统一8px圆角，字幕与核心控制保留 Semantics。连接断开、重连、恢复及低余额状态已统一为本地化 live-region，320dp/200%字体组合回归通过；仍等待深色和 VoiceOver/TalkBack 真机验收。
...
2026-08-02 当前优化总顺序：`OPT-ASR-002 单变量隔离门 -> OPT-SPK-009/010 +
...
或注销修改 session 后返回“我的”会重新读取账号状态。
