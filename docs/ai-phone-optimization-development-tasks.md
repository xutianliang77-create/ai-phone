...
当前关键路径：`部署最新 API/Gateway/ASR -> SPK-005-A -> SPK-006-A/B/C 真实指标 -> SPK-007-A/B -> SPK-008-A -> DEP-001-A`。`OPT-SPK-005/006` 只有真机和真实 session 证据通过后才能标记 `accepted`，不能用固定语料或 mock 结果替代。
