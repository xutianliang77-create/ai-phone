...
| PostgreSQL schema | `implemented` | 已有五十五段 up/down migration、tenant-first 索引、复合 FK、强制 RLS、user directory、cell/control-plane pending projection/coordination、tenant admission、opaque subject identity、企业通讯/dispatch/策略、usage/billing、knowledge/terminology、observability trace、受控审计导出、数据生命周期、Release Control、Meeting、客服、Campaign/Lead/Consent/Suppression/Country Policy、活动审批快照、Scheduler、PSTN dispatch、Marketing Agent、人工接管、Outcome 和 CRM sync 栅栏；尚无真实 migrate/restore/PITR 证据 |
...
| 公共 Primary Runtime 收敛 | `ready_for_acceptance` | 已合入上游稳定提交 `fe1c3c2`；公共31段与 enterprise 55段 manifest 由一个启动编排验证，driver、数据库身份和分权连接失败均在监听前闭合；尚无真实 PostgreSQL H3 证据 |
...
| Primary 全量切换/恢复证据 | `ready_for_acceptance` | 工具按运行时动态校验 manifest/全业务表；`c9b5be2` 历史证据为31+16/81张表，当前31+55必须重新生成签名证据；异地 WAL/PITR/H3 未通过 |
| PostgreSQL 备份和灾备 | `in_progress` | schema-v2 HMAC 结果已绑定当前 enterprise cutover、候选版本、数据库 identity 与31+55 manifest，并定义自动切换/三层 fencing/第三故障域不可变备份/PITR marker+hash 门禁；测试定义未执行，真实跨故障域、异地主机、RPO/RTO 和 H3 未通过 |
...
commit/image/topology、cutover ID、target logical ID、当前 system identifier/OID 和31+55
...
一致，且 commit、image、topology、target system identifier/OID、公共31段和 enterprise 55段 manifest 与
...
production checker 重新计算 topology/capacity/evidence 文件 hash、当前31+55 manifest 和企业 DR binding，再验
...
构建候选与文件规模静态检查；未运行测试、真实 PostgreSQL 31+55 双库、对象存储复制、控制面 route 发布、
