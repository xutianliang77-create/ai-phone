...
| PostgreSQL schema | `implemented` | 已有五十六段 up/down migration、tenant-first 索引、复合 FK、强制 RLS、user directory、cell/control-plane pending projection/coordination、tenant admission、opaque subject identity、企业通讯/dispatch/策略、usage/billing、knowledge/terminology、observability trace、受控审计导出、数据生命周期、Release Control、Meeting、客服、Campaign/Lead/Consent/Suppression/Country Policy、活动审批快照、Scheduler、PSTN dispatch、Marketing Agent、人工接管、Outcome 和 CRM sync 栅栏；尚无真实 migrate/restore/PITR 证据 |
...
| 订阅和欠费生命周期 | `in_progress` | enterprise `0056` 已形成签名 event、可恢复 command、不可变 decision、状态 API、Cell Worker、冻结 entitlement 排空和 usage period 重建代码候选；测试与真实支付/RLS/故障/财务验收未执行 |
...
| Primary 全量切换/恢复证据 | `ready_for_acceptance` | 工具按运行时动态校验 manifest/全业务表；`c9b5be2` 历史证据为31+16/81张表，当前31+56必须重新生成签名证据；异地 WAL/PITR/H3 未通过 |
| PostgreSQL 备份和灾备 | `in_progress` | schema-v2 HMAC 结果已绑定当前 enterprise cutover、候选版本、数据库 identity 与31+56 manifest，并定义自动切换/三层 fencing/第三故障域不可变备份/PITR marker+hash 门禁；测试定义未执行，真实跨故障域、异地主机、RPO/RTO 和 H3 未通过 |
...
| 租户限流和共享 Cell 公平 | `in_progress` | `0055`、四能力 Cell/tenant policy/state/request、并发/速率/有限队列、加权虚拟完成时间、runtime/operator 函数、reconcile Worker，以及 Worker Dispatch/Marketing PSTN/Screen Share admission fence 已形成；测试定义未执行，真实 RLS/并发/公平/容量/soak 未验收 |
...
形成 table count/hash/last-key hash，再汇总公共31段、企业56段 checksum、关键表、
...
commit/image/topology、cutover ID、target logical ID、当前 system identifier/OID 和31+56
...
一致，且 commit、image、topology、target system identifier/OID、公共31段和 enterprise 56段 manifest 与
...
production checker 重新计算 topology/capacity/evidence 文件 hash、当前31+56 manifest 和企业 DR binding，再验
...
构建候选与文件规模静态检查；未运行测试、真实 PostgreSQL 31+56 双库、对象存储复制、控制面 route 发布、
...
### 13.4 Tenant Admission 与共享 Cell 公平（ENT-REL-007）

`0055_enterprise_tenant_admission` 增加 Cell policy/state、Cell-Tenant state 和 tenant request。Policy 固定
四种 capability 的 Cell总并发、默认租户并发、rate/window、Cell/tenant queue 和 TTL；tenant state 保存
active/queued、weight、last virtual finish 与 rate window；request 保存稳定 resource/idempotency/hash、units、
status、virtual finish 和 lease。表全部 forced RLS，PUBLIC 无表权限；runtime 只能调用验证 `$1=current_tenant_id`
的 reserve/renew/release/active 函数。operator apply/weight/status/reconcile 函数从 PUBLIC 撤权，只授予独立 admission role。

reserve 先锁 tenant 和 Cell policy/state，以数据库时钟过期旧 queue/lease，再计算
`max(cell.virtual_time, tenant.last_finish) + units/weight`。只在 Cell active、租户 `min(entitlement, platform limit)`、
rate window 和有限队列全部允许时 admit；eligible head 按 virtual finish/queuedAt/tenant/request 稳定选择，使用
`FOR UPDATE SKIP LOCKED`。queue full/not_ready/queued 都不创建后续业务副作用。计数是请求表的可重建投影；独立
reconcile Worker 周期过期并只在漂移时更新 state，不能以 Redis/进程内 semaphore 覆盖。

Worker Dispatch 在 public capacity/grant 前 reserve；accept/authorize/heartbeat/refresh/finalize/cancel 将 admission
作为第三道 lease fence。Marketing Scheduler 在 usage hold/claim 前 reserve，PSTN prepare 前 active check，accepted/
webhook 续租到最多配置通话时长，已知失败和终态 release；未知结果只保留有界 lease。Screen Share 使用
tenant/meeting/idempotency 的确定性 share UUID，acquire/renew/resume/stop/force-stop/revocation 与 admission 同事务。

Policy manifest 绑定 candidate commit/image，要求每个 Cell 四能力齐全、key 唯一和所有数值有界；apply 使用
expectedVersion，tenant weight 只在 active/queued 均为0时修改。`enterprise:admission status` 先 reconcile 再输出
无 tenant 明细的 Cell/capability 汇总。API 通过不同的只读 observer 连接调用 aggregate readiness，最多缓存1秒；
policy未应用、version/limit漂移或计数越界时 `/health/release-ready` 失败。全局 admission 表不进入 Cell tenant 复制，任何 queued/admitted 请求阻断迁移。
当前未运行 `0055`、普通角色RLS、真实并发/权重/速率/队列、Provider故障、25/50/100或120分钟soak，保持 in_progress。

### 13.5 订阅和欠费生命周期（ENT-REL-008）

`0056_enterprise_subscription_lifecycle` 为 Subscription 增加
`active/past_due/suspended/superseded/cancelled` 受控状态，Billing Account 保持
`active/past_due/suspended/closed`。数据库 trigger 只允许 payment-failed、grace-expired、恢复/续费、注销所需
单向转换；身份、plan、seat、cycle、账期和创建时间不可改写。down migration 在存在无法由旧版本解释的状态时
失败闭合，要求先完成受控 remediation。

内部入口 `POST /internal/enterprise/billing/subscription-events` 要求配置 Provider、至少32字节独立 HMAC key、
Unix时间戳和默认300秒重放窗。签名正文使用固定字段顺序、规范 ISO 时间、tenant/subscription/provider/event ID、
event type 和 provider payload SHA-256；未知字段、名称漂移、签名篡改、过期时间、错 Provider、同 event ID 异
request hash 均在写入/副作用前拒绝。`renewed.effectiveAt` 必须等于新 period start；payment recovery 的
effective time 必须位于恢复账期内，避免提前退休当前权益。数据库只保存 hash，不保存 Provider 原始正文或密钥。

ingest transaction 锁定 tenant/billing account/subscription，原子插入 append-only event 与 pending command。
`platform_pending_work` trigger 只投影 command ID；Cell Worker 必须同时持外层 coordination generation 和内层
command lease generation。apply transaction 锁 command/event/account/subscription，检查 attempt/owner/generation/
expiry 后再写状态、entitlement、decision、账期 aggregate、audit 和 command terminal；任何一步或 lease 复核失败
整笔回滚。最多五次 attempt，过期 processing 可由更高 generation 重领。

状态矩阵为：`payment_failed` 将 active 变 past_due；`grace_expired` 将 past_due 变 suspended 并禁用当前
projection；`renewed/payment_recovered` 将旧 subscription/snapshot 变 superseded/retired，并创建新 active
subscription/snapshot/projection；`cancelled` 将账户关闭、订阅取消、权益退休。过期/逆序/状态不匹配事件只产生
`ignored` decision，不改业务真值。正常续费或恢复以 event effective time 关闭旧账期，按 ledger 中实际出现的
category/unit 调用 `rebuildUsagePeriod`，count/hash/watermark 仍来自不可变 ledger。

新 Worker/PSTN/Screen Share dispatch 已有 account/subscription/current-period guard，因此 past_due 后不再创建新
高成本动作；已接受 Provider、活动人工接管、旧 binding/ticket 不重读当前账户，可继续结束、结算和审计。
`GET /saas/v1/tenants/:tenantId/subscription/lifecycle` 只向 `billing:read` + 当前签名 route 返回账户/订阅状态、账期
和最后 event/decision，不返回 Provider payload、签名或内部 lease。企业 Web 设置页独立读取 lifecycle；即使欠费状态
没有 active entitlement，也必须展示 past_due/suspended/closed 和安全排空提示，不回退缓存或本地套餐。Provider 未配置时 `/health/release-ready`
保持503。当前未执行自动化、真实 `0056`、forced-RLS、双 Worker crash、真实支付 sandbox 或财务抽样，保持
`in_progress`。
