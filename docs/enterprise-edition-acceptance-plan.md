...
| AC-ENT-0020 | Primary 切换与恢复证据 | 公共31段/企业56段 manifest、全部业务表 count/整行 hash、关键 tenant/session/ledger/audit/consent/suppression/country-policy/approval/PSTN dispatch/Marketing handoff/release control/billing lifecycle/object 清单和增量 WAL 水位一致；源 writer fence 与 SQLSTATE 25006 写拒绝、旧 API/Worker 角会话为0、目标写探针成功；baseline/cutover/restore evidence 验签并绑定 commit/image/topology/system identifier/OID；任意单行篡改失败闭合 |
...
| AC-ENT-0041 | Scoped PSTN dispatch | 内部 dispatch 同时校验密钥、签名 route、当前 tenant/cell/epoch、claim token/generation、approval/Lead/link/Consent/Suppression/Policy/当地窗口、有效 entitlement 和剩余 lease；prepare 原子创建 scoped communication session/binding、无明文号码 Outbox 与不可变 dispatch，Provider 使用稳定 generation 幂等键；明确 accepted 只结算一次固定60秒并由 DB evidence 推进 task/binding，响应未知保持 reconciliation required 且不盲目重拨；HMAC webhook 以 event ID 去重并重验 route/generation/provider call，迟到/篡改/跨租户拒绝；Provider/Bridge/webhook/keyring/idempotency 未配置均 not_ready；Web 只读展示真实计数与 readiness，不伪造拨号成功 |
| AC-ENT-0042 | Marketing Agent | profile 只在未提交草稿可写且开场逐字包含品牌、AI 身份和营销目的；PSTN 前必须解析目标国家/locale profile、当前有效 published Term Pack/Script Template、LLM 与签名 runtime；`0046` 三表 tenant-first FK/forced RLS，run 与 dispatch 同事务并由 task transition 复核；ticket 固定 tenant/dispatch/task/session/generation/route epoch；disclosure authorize+delivered 后才可 turn；qualification 使用配置原问题，answer/objection 只引用本轮 tenant knowledge，禁承诺和额外字段拒绝；TTS authorize+delivered 后才推进；退订原子写 suppression 并结束；人工接管未实现时如实停止；Provider/证据/语言版本缺失失败闭合；Web 只展示真实配置/readiness |
| AC-ENT-0043 | Marketing 监控证据 | `campaign:read` 和签名 route 只读 Campaign/单通话 tenant-scoped 投影；最终 revision 字幕、Agent 意图/风险/引用、Provider operation/接受/接听延迟与新鲜度均源于服务端证据；跨租户、跨 Campaign/会话、旧 route 与公共 scope 篡改均拒绝；无样本/无 Provider 不补数；当前5秒快照必须明示 `streamStatus=not_configured`，只有可靠 WSS/SSE 证据后才可称流式 |
...
| AC-ENT-0048 | 单租户 Cell 迁移与回滚 | 只有 maintenance 模式和受审计全读角色可 export/cutover，rollback replace 还必须是 superuser；源 tenant 无非终态 session/dispatch、活动 lease/hold，源库只读且旧 API/Worker writer 为0，目标可写且 tenant 为空；动态计划覆盖全部 enterprise tenant 表和12类公共 tenant communication scope，任何无 selector/无主键/schema 或31+56 migration 漂移均失败；全局 control-plane/admission policy/state/request 表不进入 Cell 数据复制，活动 provision claim 或 queued/admitted admission 必须阻断迁移；签名 export 绑定 tenant、Cell、数据库身份、commit/image/topology，cutover 精确引用前序文件 hash；对象引用非0时必须有同 migration/tenant/方向/count/hash 的签名复制 receipt；目标 serializable 导入后记录、session、ledger、audit、consent、suppression、对象引用逐表 count/SHA-256 全等，homeRegion/status 不变且 route epoch 精确+1，失败目标零部分数据且不发布 route；回滚必须从当前 Cell 导出最新快照、反向全量覆盖旧 Cell、再次 epoch+1 和全表对账，不能直接启用陈旧副本；旧 route/旧 Cell 写入和 Worker claim 均拒绝 |
...
communication session、Outbox、PSTN dispatch/webhook 与首次60秒结算已在 `ENT-MKT-008` 形成静态代码候选，
但不作为 `AC-ENT-0040` 的通过证据。

`ENT-MKT-008` 按 `AC-ENT-0041` 增加十组矩阵：内部入口覆盖密钥、签名 route、tenant/region/cell/epoch 和 body
伪造；claim fence 覆盖 token、generation、lease 小于15秒和旧 task；合规复核覆盖审批、Lead/link、Consent、
Suppression、Policy/当地窗口和 entitlement；prepare 原子性覆盖 session/binding/Outbox/dispatch 任一点失败全回滚和
Outbox 无明文号码；Provider 矩阵覆盖未配置、非 HTTPS、凭据/webhook/idempotency 缺失、明确拒绝、超时/断连和
408/425/429/5xx；幂等矩阵覆盖响应丢失、同 generation 重放和不同 generation；账务矩阵覆盖 accepted 首次60秒 settle、
重复 finalize/webhook、过期 hold 和账务拒绝；webhook 矩阵覆盖签名篡改、event ID 同/异 payload、旧 route/generation/
provider call、乱序 answered/completed/failed；schema 矩阵覆盖 `0045` up/down/forward、forced RLS、复合 FK、不可删除/
身份不可改和 task transition evidence；Web/PSTN Bridge 矩阵覆盖真实 readiness/count、enterprise metadata/status sink、
Material Icons、320/600/960/1280、主题、键盘和无模拟成功。

本轮只形成代码、测试定义和静态门禁候选；未运行自动化、`0045` migration/forced-RLS、双租户、真实 PSTN Provider、
响应丢失对账、50并发或浏览器。因此 `AC-ENT-0041` 未通过，`ENT-MKT-008` 仍由外部 PSTN sandbox/凭据与正式验证
阻塞，保持 `blocked`；这些证据不替代下述 `ENT-MKT-009` Marketing Agent 独立验收。

`ENT-MKT-009` 按 `AC-ENT-0042` 增加十二组矩阵：profile 矩阵覆盖 draft/not_submitted、批准后冻结、同国家/locale
版本和幂等冲突；告知矩阵覆盖品牌/AI 身份/营销目的逐项缺失、authorize 未交付和重复回执；内容矩阵覆盖 Term Pack/
Script Template draft/review/expired/locale/country/product/purpose/effective-time；PSTN 原子性覆盖 dispatch/run 任一点失败全
回滚；ticket 矩阵覆盖签名、过期、tenant/dispatch/task/session/generation/route epoch 篡改；状态机矩阵覆盖 disclosure、
qualifying、answering、closing、终态倒退和 sequence；知识矩阵覆盖跨租户、空 evidence、citation 越界和生成期间 hash 漂移；
输出矩阵覆盖额外 thinking/tool 字段、资格问题改写、价格/付款/退款/合同/医疗/法律/金融保证；退订矩阵覆盖确定性词表、
同号码锁、不可变 suppression、任务停止和重复请求；语言矩阵覆盖只切换已配置 profile/published 内容，缺目标语言失败闭合；
TTS 矩阵覆盖 authorize/delivered、旧 generation 和中断后迟到 playout；Web/Provider 矩阵覆盖真实 readiness、Material Icons、
主题/响应式/键盘以及未配置 LLM/runtime/人工接管不显示成功。

当前只形成代码、测试定义和静态门禁候选；未运行 Vitest/API/Repository、`0046` migration/down/forward、普通角色
...
`ENT-MKT-011` 按 `AC-ENT-0044` 增加十组矩阵：策略矩阵覆盖 draft/not_submitted、active queue/PSTN channel、
超时边界、callback delay、expectedVersion 和幂等冲突；审批矩阵覆盖缺策略、失活资源、snapshot/hash 漂移；
物化矩阵覆盖 handoff turn 未 delivered、dispatch/run/session/策略错绑、原子回滚和重复交付；claim 矩阵复用
`AC-ENT-0030` 的唯一 active claim、lease/改派/跨租户拒绝；AI fence 矩阵覆盖 active/handoff_requested/
cancelled/terminal run 与旧 Worker TTS；Provider readiness 矩阵覆盖未配置、非 HTTPS、凭据、幂等、operator join 和
300ms 保证；回执矩阵覆盖超时、HTTP 失败、非法 JSON、超时 stop latency、缺时间或 join 早于 stop；工作台
矩阵覆盖 claim/AI fence/media 三种证据分层显示；超时矩阵覆盖无 claim、有 claim、end/callback 收敛和
`physicalProviderAction=not_verified`；恢复/容量矩阵覆盖 API/DB/Worker 重启、迟到 Provider 回执和并发接管。

当前只形成代码、未运行测试定义和静态门禁候选；未运行 Vitest/API/Repository、`0047` migrate/down/forward、
普通角色 forced-RLS、双租户、真实 PostgreSQL/PSTN Provider/坐席加入、300ms 媒体中断、浏览器或容量验收。
因此 `AC-ENT-0044` 未通过，`ENT-MKT-011` 保持 `in_progress`；`callback_required` 不是回拨已排程/已接通证据。

`ENT-MKT-012` 按 `AC-ENT-0045` 增加九组矩阵：角色矩阵覆盖 owner/admin/marketing manager/marketing member
读写、auditor 只读和其他角色拒绝；租户矩阵覆盖 body tenantId、跨 Campaign/task/dispatch/session、旧 route 与公共
scope 篡改；终态矩阵覆盖 task/dispatch completed/failed、accepted/answered 非终态、run active/terminal 与 handoff
各状态；证据矩阵覆盖字幕最大 revision、已交付 turn、跨租户/跨通话 ID、重复选择、原文不落 evidence/audit 和 hash
重算；分类矩阵覆盖潜在线索/预约的客户证据、退订 suppression、无效号码 allowlist failure code、真实失败和意向矛盾；
幂等/并发矩阵覆盖同键同 hash 重放、异 hash 冲突、同 task 多键竞争和 legacy_unverified 占位；action 矩阵覆盖0/1条、
kind/due/evidence hash deferred 匹配、append-only 和 requested 非外部完成；Web 矩阵覆盖 Material Icons、只读角色、
无证据禁用、empty/403/404/422/503、320/600/960/1280、浅深色和键盘；恢复矩阵覆盖 API/DB 重启、事务回滚和读取稳定性。

当前只形成代码、未运行测试定义和静态门禁候选；未运行 Vitest/API/Repository、`0048` migrate/down/forward、
普通角色 forced-RLS、双租户、真实 PostgreSQL/PSTN/Agent 通话、浏览器或容量验收。因此 `AC-ENT-0045` 未通过，
`ENT-MKT-012` 保持 `in_progress`；内部 `requested` 不是外部 Provider 成功证据。下一项为 `ENT-MKT-013` CRM Adapter。

`ENT-MKT-013` 按 `AC-ENT-0046` 增加八组矩阵：角色/租户矩阵覆盖 owner/admin/marketing 写、auditor 只读、
跨 Campaign/Outcome/body/header/旧 route 拒绝；配置矩阵覆盖缺 Provider、租户绑定、OAuth、login URL、API version、
sObject、External ID/payload field、keyring 及配置漂移；幂等/并发矩阵覆盖同键重放、异 hash、单 Outcome 多键竞争和
稳定 External ID；加密矩阵覆盖 AAD 篡改、密钥轮换、未知 key ID、密文搬运和明文扫描；Provider 矩阵覆盖 create/update
upsert、401 token 更新、400/403/404 终态拒绝、408/409/425/429/5xx/超时重试、PATCH 响应丢失、GET mismatch 与无效
Record ID；Worker 矩阵覆盖 claim 后崩溃、publish 后 finalize 前崩溃、attempt 乱序、重复 receipt 和旧 fingerprint；业务
隔离矩阵覆盖 CRM 故障时 task/dispatch/session/usage settle/Outcome/action 均不回滚或重复；Web 矩阵覆盖 pending/synced/
failed、只读、not_ready、外链、320/600/960/1280、浅深色、键盘和无 secret/source-map 泄露。
...
- 验收 commit 锁定的公共31段 manifest（基线从 `fe1c3c2` 演进）与 enterprise 56段 migration manifest 在隔离企业数据库从空库完整执行；两个 manifest 的顺序、checksum、schema verify 和 down/forward 策略均有证据，不能只跑其中一套。
...
- 以真实31+56 staging 双库执行 `export -> object receipt -> cutover -> reconcile`，确认全部 enterprise tenant 表、
...
- staging startup 必须拒绝 local evidence、签名篡改、错误 cutover/target ID、错误 commit/image/topology、错误 system identifier/OID、缺 baseline 引用、未清退 writer 或任一31+56 migration 漂移。维护工具只验证 fence，不自动执行 promote 或隔离旧主。
...
- Worker 同 Cell 多实例用数据库 `SKIP LOCKED` + owner/generation/lease 竞争；验证只有一个有效 claim、续租失败不开始新副作用、旧 generation 不能 release/finalize、崩溃后更高 generation 到期重领；单条 finalize 失败不阻断同批其他记录，poll 故障后继续下一轮，所有 Provider 投递复用 event/job ID 幂等键。
...
`ENT-DATA-005` 当前只形成 `AC-ENT-0048` 的维护命令、签名证据、流式导入、回滚和静态测试定义；本轮未运行
测试、真实 PostgreSQL、对象复制、路由发布或故障注入，因此不能升级为 `ready_for_acceptance`，也不构成 H3 证据。
...
`ENT-REL-004` 当前已在本机官方 PostgreSQL 16.14 完成公共31段+enterprise 53段空库 migration、
`53 -> 52 -> 51 -> 53` down/forward、普通角色双租户 RLS、两个连接池同 operation 竞争、
open/half-open/probe/recovery/kill、追加证据不可变和两个 API 进程同库状态读取；定向6文件32项、
API全量233文件829项、Enterprise Web 13文件74项和全 Node 433文件1609项通过。
该单节点机制证据不替代真实 Support/OCR/PSTN Provider 故障、kill 生效延迟 SLO、告警、独立 reviewer、
...
`ENT-REL-006` 按 `AC-ENT-0055` 执行七组矩阵：migration/RLS 矩阵覆盖 `0054` up/down/forward、普通
control-plane role、tenant role 伪造 worker GUC、投影 insert/update/delete 和 instance/claim 非法 generation；
API 原子矩阵在 tenant/member/job/Directory/projection 每一步注入失败并确认全有或全无，重复100次同 key 只有一个
tenant/job；并发矩阵以至少两个进程、50个任务证明 `SKIP LOCKED` 单 claim、同 worker ID 拒绝、续租和排空；
崩溃矩阵分别在 API commit 后、Provider 前/后、finalize 前 kill -9，确认到期重领且区域实体/tenant/job/audit
只完成一次；状态矩阵覆盖副本不足、候选混跑、过期实例、数据库时钟 backlog 边界和不泄露 tenant/连接信息；
控制面中断矩阵确认新开通/套餐变更503无半条写入，已建立会议/客服/外呼继续安全结束、账务/审计收敛；滚动发布/
网络分区矩阵确认旧 generation 永远不能恢复写入或激活 tenant。

当前只形成 migration、session/repository/Worker、API guard、status CLI、运行手册和未执行测试定义。本轮按要求
未运行自动化、真实31+56 PostgreSQL、两个实例、故障注入、区域 provisioner 或数据面自治验证，
`AC-ENT-0055` 未通过，`ENT-REL-006` 保持 `in_progress`，不能把 config `configured` 或单机 status 称为 HA。

### 12.3 租户限流与共享 Cell 公平验收

`ENT-REL-007` 按 `AC-ENT-0056` 执行八组矩阵：schema/权限覆盖 `0055` up/down/forward、forced-RLS、
runtime函数tenant绑定、operator函数PUBLIC撤权/专用GRANT和直接表写；policy矩阵覆盖四能力缺失、候选漂移、
expectedVersion、数值上下界、disabled和权重活动冲突；并发矩阵覆盖Cell/tenant上限及entitlement较小值；rate/queue
矩阵覆盖窗口前后1ms、Cell/tenant queue满、TTL和同键重放/异hash；公平矩阵以权重1/2/4、大租户持续与小租户突发
验证eligible-head无头阻塞及批准SLO；生命周期矩阵覆盖Worker accept/heartbeat/refresh/finalize/cancel、PSTN
prepare/accepted/unknown/webhook、Screen Share acquire/renew/stop/revoke；故障矩阵在reserve/业务写/Provider/renew/release
各阶段kill并用reconcile重建准确计数；容量矩阵执行25/50/100与120分钟真实混合流量，核对拒绝、OOM、队列、Provider
副作用、usage/ledger和终态。

当前只形成 migration、runtime/operator functions、policy parser/admin/worker、三条业务集成、health/release gate、
运行手册和未执行测试定义。本轮未运行真实31+56 PostgreSQL、专用角色GRANT/RLS、双租户公平、媒体/Provider、
阶梯/soak或独立reviewer，`AC-ENT-0056` 未通过，`ENT-REL-007` 保持 `in_progress`。

### 12.4 订阅和欠费生命周期验收

`ENT-REL-008` 按 `AC-ENT-0057` 执行七组矩阵：schema/RLS矩阵覆盖 `0056` up/down/forward、普通tenant、
Cell Worker、PUBLIC、跨租户ID和三张新表直接写；edge矩阵覆盖Provider配置、HMAC长度、规范字段、时间窗、错签名、
重放、同event异hash和payload上限；状态矩阵覆盖renew/payment_failed/grace_expired/payment_recovered/cancelled、逆序、
重复和非法跳转；权益矩阵确认新dispatch立即阻断、进行中接管/已接受通话/安全结束可排空、恢复生成新版本且旧ticket
不能复活；Worker矩阵在外层claim、内层claim、状态写、账期重建、audit和command finalize各阶段kill并验证更高
generation单次收敛；账务矩阵逐category/unit核对raw event、ledger、adjustment、aggregate count/hash/watermark与财务
样本；API/迁移矩阵覆盖billing:read route、敏感字段、provider not_ready、活动command阻断Cell迁移和终态历史迁移。

当前只形成migration、Repository/runtime、HMAC入口、Cell Worker、状态API、health gate和未执行测试定义。本轮未运行
自动化、真实31+56 PostgreSQL/RLS、支付sandbox、双Worker故障、业务会话排空或财务抽样，`AC-ENT-0057` 未通过，
`ENT-REL-008` 保持 `in_progress`，不得宣称企业订阅/欠费生产门禁通过。
